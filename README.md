...
ide-code-assistant/ ├── api.py # Основной файл приложения FastAPI ├── config_storage.py # Управление конфигурацией и API-ключами ├── main.py # Консольное приложение для взаимодействия с OpenAI API ├── requirements.txt # Список зависимостей проекта ├── test_api.py # Тесты для приложения ├── .env # Файл с переменными окружения (не допускать в репозиторий) ├── .gitignore # Список файлов и папок, игнорируемых Git ├── pytest.ini # Конфигурация Pytest ├── README.md # Документация проекта └── venv/ # Виртуальное окружение (не включать в репозиторий)
...
Примечание: Эта документация предназначена для предоставления подробной информации о настройке и использовании приложения. Если у вас есть вопросы или обнаружили проблемы, пожалуйста, создайте issue в репозитории проекта или свяжитесь с автором.
